	...
	public void service(
		...
		if (portlet != null) {
			...
			if (requestURI != null) {
				...
				int servletContextNameIndex = servletContextName.indexOf(
					StringPool.SLASH);

				if (servletContextNameIndex != -1) {
					requestURITemp = servletContextName.substring(
						servletContextNameIndex, servletContextName.length());

					servletContextName = servletContextName.substring(
						0, servletContextNameIndex);
				}
			}
		}
		...
		if (servletContext == null) {
			PortalUtil.sendError(
				...
				new IllegalArgumentException(
					"No application mapped to the path " + requestURI),
				request, response);
	...
	private void _service(
		...
		try {
			...
			PortalUtil.sendError(
				HttpServletResponse.SC_NOT_FOUND,
				new IllegalArgumentException(
					"No servlet or resource mapped to path: " + requestURI),
					request, response);
		}
		finally {
