...
create unique index IX_12566EC2 on Company (mx);
...
create unique index IX_E597322D on IGImage (custom1ImageId);
create unique index IX_D9E0A34C on IGImage (custom2ImageId);
create index IX_4438CA80 on IGImage (folderId);
create index IX_BCB13A3F on IGImage (folderId, name);
create unique index IX_64F0B572 on IGImage (largeImageId);
create unique index IX_D3D32126 on IGImage (smallImageId);
...
create unique index IX_25FFB6FA on JournalTemplate (smallImageId);
...
create unique index IX_FAD05595 on Layout (dlFolderId);
...
create unique index IX_23922F7D on Layout (iconImageId);

create index IX_A40B8BEC on LayoutSet (groupId);
create unique index IX_48550691 on LayoutSet (groupId, privateLayout);
create unique index IX_5ABC2905 on LayoutSet (virtualHost);
...
create unique index IX_ED7CF243 on PasswordPolicyRel (passwordPolicyId, classNameId, classPK);
...
create unique index IX_D699243F on PortletItem (groupId, name, portletId, classNameId);
...
create unique index IX_7311E812 on SCProductEntry (repoGroupId, repoArtifactId);

create unique index IX_AE8224CC on SCProductScreenshot (fullImageId);
create index IX_467956FD on SCProductScreenshot (productEntryId);
create unique index IX_DA913A55 on SCProductScreenshot (productEntryId, priority);
create unique index IX_6C572DAC on SCProductScreenshot (thumbnailId);

create unique index IX_7020130F on SCProductVersion (directDownloadURL);
...
create unique index IX_903DC750 on ShoppingItem (largeImageId);
create unique index IX_D217AB30 on ShoppingItem (mediumImageId);
create unique index IX_FF203304 on ShoppingItem (smallImageId);
...
create unique index IX_F474FD89 on ShoppingOrder (ppTxnId);
...
create unique index IX_1271F25F on SocialActivity (mirrorActivityId);
...
create unique index IX_6EF03E4E on User_ (companyId, defaultUser);
...
create unique index IX_A9ED7DD3 on User_ (openId);
