...
for any [Mojo](https://maven.apache.org/general.html#What_is_a_Mojo)'s found in
your project.
...
[`mavenEmbedderClasspath`](#mavenembedderclasspath) | [`configurations.mavenEmbedder`](#mavenembedderdependency)
...
`buildPluginDescriptor`, in order to let `processResources` copy the
newly-generated files in the `buildPluginDescriptor.outputDir` directory.
...
`pom.xml` file based on the project and then invoking [Maven Embedder](http://maven.apache.org/ref/3.3.9/maven-embedder/)
in order to build the Maven plugin descriptor.
...
<a name="mavenembedderclasspath"></a>`mavenEmbedderClasspath` | `FileCollection` | `null` | The classpath used to invoke Maven Embedder.
`mavenEmbedderMainClassName` | `String` | `"org.apache.maven.cli.MavenCli"` | The Maven Embedder main class name.
`mavenPluginPluginVersion` | `String` | `"3.4"` | The version of the [Maven Plugin Plugin](https://maven.apache.org/plugin-tools/maven-plugin-plugin/) to use in order to generate the plugin descriptor for the project.
...
`pomRepositories` | `Map<String, Object>` | `["liferay-public": "http://repository.liferay.com/nexus/content/groups/public"]` | The name and the URL of the remote repositories. It adds [`repositories.repository`](http://maven.apache.org/ref/3.3.3/maven-model/maven.html#class_repository) elements in the generated `pom.xml` file.
...
<a name="proxypassword"></a>`proxyPassword` | `String` | `null` | The password to use in order to access a protected proxy server. It sets the value of the [`proxies.proxy.password`](https://maven.apache.org/settings.html#Proxies) element in the generated `settings.xml` file.
...
<a name="proxyuser"></a>`proxyUser` | `String` | `null` | The user name to use in order to access a protected proxy server. It sets the value of the [`proxies.proxy.username`](https://maven.apache.org/settings.html#Proxies) element in the generated `settings.xml` file.
...
dependency to the 3.3.9 version of Maven Embedder. It is possible to override
this setting and use a specific version of the tool by manually adding a
dependency to the `mavenEmbedder` configuration:
