...
The Change Log Builder Gradle plugin allows you to generate and maintain a
change log file based on the Git commits on your project. A change log file
...
range is always the tip of the current branch, while the start is:

- the first time `buildChangeLog` is executed: the most recent commit from at least two years ago;
- otherwise: the range end of the last line in the change log.
...
`gitDir` | `File` | `project.rootDir` | The base directory where to start searching for the `.git` directory. The search proceeds in all the ancestors of the directory specified.
