...
ddm.radio = function(opt_data, opt_ignored) {
  ...
  for (var optionIndex22 = 0; optionIndex22 < optionListLen22; optionIndex22++) {
    ...
    output += ((! opt_data.inline) ? '<div class="radio">' : '') + '<label class="radio-default' + soy.$$escapeHtml(opt_data.inline ? ' radio-inline' : '') + ' radio-option-' + soy.$$escapeHtml(optionData22.value) + '" for="' + soy.$$escapeHtml(optionData22.value) + '"><input class="field" dir="' + soy.$$escapeHtml(opt_data.dir) + '" ' + ((opt_data.readOnly) ? 'disabled' : '') + ' id="' + soy.$$escapeHtml(optionData22.value) + '" name="' + soy.$$escapeHtml(opt_data.name) + '" ' + soy.$$escapeHtml(optionData22.status) + ' type="radio" value="' + soy.$$escapeHtml(optionData22.value) + '" /> ' + soy.$$escapeHtml(optionData22.label) + '</label>' + ((! opt_data.inline) ? '</div>' : '');
  }
