...
import static com.liferay.adaptive.media.image.rest.client.internal.util.AdaptiveMediaImageTestUtil.TEST_AUTH;

import com.google.gson.JsonArray;

import com.liferay.adaptive.media.image.rest.client.internal.util.AdaptiveMediaImageTestUtil;
...
public class AdaptiveMediaImageJaxRsMediaTest {
	...
	public void testGettingAdaptiveByInvalidConfigReturns404IfParam() {
		String id = AdaptiveMediaImageTestUtil.getRandomUuid();
	...
	private Invocation.Builder _getAdaptiveMediaRequest(
		...
		return AdaptiveMediaImageTestUtil.getMediaRequest(webTarget -> {
	...
	static {
		...
		long groupId = AdaptiveMediaImageTestUtil.getGroupId();

		long nonAdaptiveMediaFolderId = AdaptiveMediaImageTestUtil.getFolderId(
			groupId, "Non Adaptive Media");
		long adaptiveMediaFolderId = AdaptiveMediaImageTestUtil.getFolderId(
			groupId, "Adaptive Media");

		_nonAdaptiveFileEntryIds = AdaptiveMediaImageTestUtil.getFileEntryIds(
			5, groupId, "image-without-%d.jpeg", nonAdaptiveMediaFolderId);

		_adaptiveFileEntryIds = AdaptiveMediaImageTestUtil.getFileEntryIds(
