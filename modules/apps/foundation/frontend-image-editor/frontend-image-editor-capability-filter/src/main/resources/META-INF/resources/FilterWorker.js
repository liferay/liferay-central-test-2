...
onmessage = function(event) {
	...
	var controlPoints = getControlPoints(filter);

	if (controlPoints) {
		var dataLength = imageData.data.length;

		if (dataLength) {
			importScripts('FilterHelper.js');
...
/**
 * Gets the arrays (r,g,b) of control points for a given filter
 *
 ...
 * @see  http://www.instructables.com/id/How-to-make-Instagram-Filters-in-Photoshop
 */
var getControlPoints = function(filter) {
	var controlPoints;
 ...
 * filters that are expected to return grayscale images.
 * @type {Array}
 */
