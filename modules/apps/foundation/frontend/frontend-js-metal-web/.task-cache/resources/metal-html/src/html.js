define("frontend-js-metal-web@1.0.0/metal-html/src/html", ['exports', 'metal/src/metal'], function (exports, _metal) {
	'use strict';
	...
	var html = function () {
		...
		html.collapseBreakingSpaces_ = function collapseBreakingSpaces_(htmlString) {
			return _metal.string.collapseBreakingSpaces(htmlString);
		};
		...
		html.preserveInterval_ = function preserveInterval_(htmlString, start, end, preserved) {
			var blockId = '%%%~BLOCK~' + _metal.core.getUid() + '~%%%';
			preserved[blockId] = htmlString.substring(start, end);
			return _metal.string.replaceInterval(htmlString, start, end, blockId);
		};
