...
public class ActionUtil {
	...
	public static void getWorkflowTask(
			LiferayPortletRequest request, long workflowTaskId)
		throws Exception {
	...
	public static void searchWorkflowTasks(
		...
		if (tabs.equals(_COMPLETED)) {
			List<WorkflowTask> workflowTasks = searchTasksAssignedToMe(
				request, company, user, currentURLObj, assetTypes, true);

			tasks.addAll(workflowTasks);
		}
		else if (tabs.equals(_PENDING)) {
			List<WorkflowTask> workflowTasks =
				searchPendingTasksAssignedToMyRoles(
					request, company, user, currentURLObj, assetTypes);

			tasks.addAll(workflowTasks);

			workflowTasks = searchTasksAssignedToMe(
				request, company, user, currentURLObj, assetTypes, false);
		...
		request.setAttribute(
			WebKeys.WORKFLOW_TASK_DISPLAY_CONTEXT, displayContext);
	}
	...
	private static List<WorkflowTask> searchPendingTasksAssignedToMyRoles(
		...
		if (searchTerms.isAdvancedSearch()) {
			total = WorkflowTaskManagerUtil.searchCount(
				company.getCompanyId(), user.getUserId(), searchTerms.getName(),
	...
	private static List<WorkflowTask> searchTasksAssignedToMe(
		...
		if (completedTasks) {
			...
			request.setAttribute(
				WebKeys.WORKFLOW_MY_COMPLETED_TASKS, searchContainer);
		}
		else {
			searchContainer.setEmptyResultsMessage(
			...
			request.setAttribute(
				WebKeys.WORKFLOW_PENDING_TASKS_ASSIGNED_TO_ME, searchContainer);
		}
	...
	private static final String _THERE_ARE_NO_COMPLETED_TASKS =
		"there-are-no-completed-tasks";

	private static final String _THERE_ARE_NO_PENDING_TASKS_ASSIGNED_TO_YOU =
	...
	private static final String _WITH_THE_SPECIFIED_SEARCH_CRITERIA =
