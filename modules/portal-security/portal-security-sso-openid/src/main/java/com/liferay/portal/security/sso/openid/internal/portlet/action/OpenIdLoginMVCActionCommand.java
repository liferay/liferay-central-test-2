 ...
 * 
 * <ol>
 * <li>Discover the OpenID provider's XRDS document by performing HTTP GET on
 * the user's OpenID URL. This document tells Liferay what the URL of the OpenID
 * provider is.</li>
 * 
 * <li>Retrieve the OpenID provider's authentication URL which is provided by
 ...
 * in step 7).</li>
 * 
 * <li>Search for an existing Liferay Portal user with the user provided OpenID.
 * </li>
 * 
 * <li>If found, redirect the browser to the OpenID authentication request URL
 * and wait for the browser to be redirected back to Liferay Portal when all
 * steps repeat. Otherwise, ...</li>
 * 
 * <li>Generate a valid Liferay Portal user screen name based on their OpenID
 ...
 * the browser to the OpenID authentication request URL. Otherwise, ...</li>
 * 
 * <li>Enrich the OpenID authentication request URL with a request for specific
 * attributes (the user's <code>fullname</code> and <code>email</code>). Then
 * redirect the browser to the enriched OpenID authentication request URL.</li>
 * 
 * <li>Upon returning from the OpenID provider’s authentication process, the
 * MVCActionCommand finds the URL parameter <code>cmd</code> set to
 * <code>read</code> (see step 2).</li>
 * 
 * <li>The request is verified as being from the same OpenID provider.</li>
 * 
 * <li>If the attributes requested in step 6 are not found, then the web browser
 * is redirected to the Create Account page where the missing information must
 * be entered before a Liferay Portal user can be created. Otherwise, ...</li>
 * 
 * <li>The attributes are used to create a Liferay Portal user and the HTTP
