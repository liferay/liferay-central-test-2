...
abstract class Generator {
  ...
  /**
   * @param  bldr both input and output.  non <code>null</code>.  modified in
   *         place.
   * @return <code>true</code> iff there are more instances of the generator's
   *         field to generate.  If a generator is exhausted, generating a new
   *         value of a larger field may allow it to continue, so a month
   *         generator that runs out of months at 12, may start over at 1 if
   *         called with a bldr with a different year.
   * @throws IteratorShortCircuitingException when an iterator reaches a
   *         threshold past which it cannot generate any more dates. This
   *         indicates that the entire iteration process should end.
   */
