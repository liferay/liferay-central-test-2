...
public class DDMStructurePersistenceImpl extends BasePersistenceImpl<DDMStructure>
	...
	public static final FinderPath FINDER_PATH_FIND_BY_CLASSNAMEID = new FinderPath(DDMStructureModelImpl.ENTITY_CACHE_ENABLED,
			DDMStructureModelImpl.FINDER_CACHE_ENABLED, FINDER_CLASS_NAME_LIST,
			"findByClassNameId",
			new String[] {
				Long.class.getName(),
	...
	public DDMStructure fetchByPrimaryKey(long structureId)
		...
		if (ddmStructure == _NULL_PLACE_HOLDER) {
			return null;
		}
		...
		if (ddmStructure == null) {
			...
			try {
				...
				if ((!hasError) && (ddmStructure == null)) {
					EntityCacheUtil.putResult(DDMStructureModelImpl.ENTITY_CACHE_ENABLED,
						DDMStructureImpl.class, structureId, _NULL_PLACE_HOLDER);
				}
				else {
					cacheResult(ddmStructure);
				}
	...
	public List<DDMStructure> filterFindByGroupId(long groupId, int start,
		...
		String sql = InlineSQLHelperUtil.replacePermissionCheck(query.toString(),
				DDMStructure.class.getName(), _FILTER_COLUMN_PK, groupId);
	...
	public List<DDMStructure> findByClassNameId(long classNameId, int start,
		...
		List<DDMStructure> list = (List<DDMStructure>)FinderCacheUtil.getResult(FINDER_PATH_FIND_BY_CLASSNAMEID,
				finderArgs, this);
	...
	public List<DDMStructure> findByG_N_D(long groupId, String name,
		...
		List<DDMStructure> list = (List<DDMStructure>)FinderCacheUtil.getResult(FINDER_PATH_FIND_BY_G_N_D,
				finderArgs, this);
	...
	private static final String _FILTER_SQL_SELECT_DDMSTRUCTURE_WHERE = "SELECT {ddmStructure.*} FROM DDMStructure ddmStructure WHERE ";
	...
	private static final DDMStructure _NULL_PLACE_HOLDER = new DDMStructureImpl();
