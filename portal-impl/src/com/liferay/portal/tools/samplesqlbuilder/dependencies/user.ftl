...
<#if organizations??>
	<#list organizations as organization>
		insert into Users_Orgs (userId, organizationId) values (${user.userId}, ${organization.organizationId});
	</#list>
</#if>

<#if userGroupRoles??>
    <#list userGroupRoles as userGroupRole>
		insert into UserGroupRole (userId, groupId, roleId) values (${user.userId}, ${userGroupRole.groupId}, ${userGroupRole.roleId});
    </#list>
</#if>