...
insert into User_ (uuid_, userId, companyId, createDate, modifiedDate, defaultUser, contactId, password_, passwordEncrypted, passwordReset, screenName, emailAddress, greeting, firstName, lastName, loginDate, failedLoginAttempts, agreedToTermsOfUse, active_) values ('${portalUUIDUtil.generate()}', ${user.userId}, ${companyId}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, <#if user.defaultUser>TRUE<#else>FALSE</#if>, ${contact.contactId}, 'test', FALSE, FALSE, '${user.screenName}', '${user.emailAddress}', 'Welcome ${contact.fullName}!', '${contact.firstName}', '${contact.lastName}', CURRENT_TIMESTAMP, 0, TRUE, TRUE);
