...
<#list finderColsList as finderCol>
	...
	<#if !finderCol.isPrimitiveType()>
		...
		<#elseif finderCol.comparator == "<>" || finderCol.comparator = "!=">
			"${entity.alias}<#if entity.hasCompoundPK() && finderCol.isPrimary()>.id</#if>.${finderCol.DBName} IS NOT NULL${finderColConjunction}"
		<#else>
			"${entity.alias}<#if entity.hasCompoundPK() && finderCol.isPrimary()>.id</#if>.${finderCol.DBName} ${finderCol.comparator} NULL${finderColConjunction}"
		</#if>
	...
	<#if finderCol.type == "String" && !finderCol.isCaseSensitive()>
		<#if entity.hasCompoundPK() && finderCol.isPrimary()>
			<#assign finderColExpression = "lower(" + entity.alias + ".id." + finderCol.DBName + ") " + finderCol.comparator + " lower(CAST_TEXT(?))">
		<#else>
			<#assign finderColExpression = "lower(" + entity.alias + "." + finderCol.DBName + ") " + finderCol.comparator + " lower(CAST_TEXT(?))">
		</#if>
	<#else>
		<#if entity.hasCompoundPK() && finderCol.isPrimary()>
			<#assign finderColExpression = entity.alias + ".id." + finderCol.DBName + " " + finderCol.comparator + " ?">
		<#else>
