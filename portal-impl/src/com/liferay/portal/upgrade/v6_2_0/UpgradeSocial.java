...
public class UpgradeSocial extends UpgradeProcess {
	 ...
	 *   3) How to generate extra data. See #createExtraDataJSONObject()
	 *
	 * The ExtraData framework works with a list of ExtraDataFactories.
	 * For each one, a query to SocialActivity table is built and run (1). For
	 * each SocialActivity tuple returned by the query, the extra data for it is
	 * generated by querying the appropriate entity as dictated by the Factory
	 * (2), then building the extra data from that entity tuple (3). This data
	 * is then used to update the original SocialActivity tuple, so that the
	 * Social Activity Interpreters can find what they need after the upgrade.
	 */
