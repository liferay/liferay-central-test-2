...
public class HtmlImplTest {
	...
	public void testExtraction() {
		assertEquals("whitespace removal", htmlImpl.extractText("   whitespace \n   <br/> removal   "));
		assertEquals("script removal", htmlImpl.extractText("script <script>   test   </script> removal"));
		assertEquals("HTML attribute removal", htmlImpl.extractText("<h1>HTML</h1> <i>attribute</i> <strong>removal</strong>"));
		assertEquals("onclick removal", htmlImpl.extractText("<div onclick=\"honk()\">onclick removal</div>"));
	}
	...
	public void testStripBetween() {
		...
		assertEquals("test--test", 
				htmlImpl.stripBetween("test-<honk>thiswillbestripped</honk>-test", "honk"));
		assertEquals("test--test", 
				htmlImpl.stripBetween("test-<honk attribute=\"value\">thiswillbestripped</honk>-test", "honk"));
		assertEquals("test-test-test",
				htmlImpl.stripBetween("test-test-test", "test"));
		assertEquals("works across  lines",
				htmlImpl.stripBetween("works across <honk>\r\n a number of </honk> lines", "honk"));
		assertEquals("test-test-test",
				htmlImpl.stripBetween("test-test-test", "test"));
		assertEquals("multiple occurrences, multiple indeed",
				htmlImpl.stripBetween("multiple <a>many</a>occurrences, multiple <a>HONK</a>indeed", "a"));
		assertEquals("self-closing <test/> is unhandled",
				htmlImpl.stripBetween("self-closing <test/> is unhandled", "test"));
		assertEquals("self-closing <test /> is unhandled",
				htmlImpl.stripBetween("self-closing <test /> is unhandled", "test"));
	}
	...
	public void testStripComments() {
		...
		assertEquals("test", htmlImpl.stripComments("te<!--  bla -->s<!-- bla bla -->t"));
