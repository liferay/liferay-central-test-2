...
public class AuthenticationUtils implements CallbackHandler {
	...
	private static final String WS_SECURITY_INFO = "<deployment xmlns='http://xml.apache.org/axis/wsdd/' xmlns:java='http://xml.apache.org/axis/wsdd/providers/java'>"
		...
		+ "               <parameter name='passwordCallbackClass' value='com.liferay.portlet.alfrescocontent.util.AuthenticationUtils'/>"
	...
	public static void startSession(String alfrescoWebClientURL,
		...
		try {
			...
			if (exception instanceof AuthenticationFault) {
				...
				throw new WebServiceException("Error starting session.",
					exception);
			}
		}
	}
	...
	public static void endSession(String alfrescoWebClientURL) {
		...
		if (ticket != null) {
			try {
				...
				throw new WebServiceException("Error ending session.",
					exception);
			}
		}
	}
	...
	public static EngineConfiguration getEngineConfiguration() {
		return new FileProvider(new ByteArrayInputStream(WS_SECURITY_INFO
			.getBytes()));
	}
}