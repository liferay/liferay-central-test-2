...
if (PropsValues.PERMISSIONS_USER_CHECK_ALGORITHM == 6) {
	...
	else if ((role.getType() == RoleConstants.TYPE_ORGANIZATION) || (role.getType() == RoleConstants.TYPE_SITE) || (role.getType() == RoleConstants.TYPE_PROVIDER)) {
...
for (int i = 0; i < results.size(); i++) {
	...
	if (role.getType() == RoleConstants.TYPE_REGULAR) {
		...
		groups = GroupLocalServiceUtil.search(company.getCompanyId(), new long[] {PortalUtil.getClassNameId(Company.class), PortalUtil.getClassNameId(Group.class), PortalUtil.getClassNameId(Organization.class), PortalUtil.getClassNameId(UserPersonalCommunity.class)}, null, null, groupParams, QueryUtil.ALL_POS, QueryUtil.ALL_POS);
