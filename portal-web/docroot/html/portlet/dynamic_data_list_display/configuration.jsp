...
<aui:form action="<%= configurationURL %>" method="post" name="fm1">
	...
	<c:if test="<%= selRecordSet != null %>">
		...
		<aui:fieldset label="templates">
			<aui:select helpMessage="select-the-list-template-used-to-diplay-the-list-records" label="list-template" name="listTemplateId" onChange='<%= "document." + renderResponse.getNamespace() + "fm." + renderResponse.getNamespace() + "listTemplateId.value = this.value;" %>'>
				...
				for (DDMTemplate template : templates) {
					...
					if (listTemplateId == template.getTemplateId()) {
						selected = true;
					}
				%>
			...
			<aui:select helpMessage="select-the-detail-template-used-to-add-records-to-the-list" label="detail-template" name="detailTemplateId" onChange='<%= "document." + renderResponse.getNamespace() + "fm." + renderResponse.getNamespace() + "detailTemplateId.value = this.value;" %>'>
				<aui:option label="default" value="<%= 0 %>" />
				...
				for (DDMTemplate template : templates) {
					...
					if (detailTemplateId == template.getTemplateId()) {
						selected = true;
					}
				%>
...
<aui:form action="<%= configurationURL %>" method="post" name="fm">
	...
	<aui:input name="preferences--detailTemplateId--" type="hidden" value="<%= detailTemplateId %>" />
	<aui:input name="preferences--listTemplateId--" type="hidden" value="<%= listTemplateId %>" />
...
<aui:script>
	Liferay.provide(
		...
		function(recordSetId, recordSetName) {
			...
			document.<portlet:namespace />fm.<portlet:namespace />detailTemplateId.value = "";
			document.<portlet:namespace />fm.<portlet:namespace />listTemplateId.value = "";
