...
<div class="body-row">
	<div class="folder-display-style aui-liferaylistview-content" <%= refreshFolders ? "data-refresh-folders=\"true\"" : StringPool.BLANK %> id="<portlet:namespace />folderContainer">
				...
				<c:when test="<%= (folderId == DLFolderConstants.DEFAULT_PARENT_FOLDER_ID) && (parentFolderId == DLFolderConstants.DEFAULT_PARENT_FOLDER_ID && showRootFolder) %>">
					...
					<li class="folder <%= (navigation.equals("documents-home") && (fileEntryTypeId == -1)) ? "selected" : StringPool.BLANK %>">
						...
						<c:if test="<%= (foldersCount > 0) %>">
							<a class="expand-folder" data-refresh-folders="<%= Boolean.TRUE.toString() %>" data-resource-url="<%= viewDocumentsHomeFoldersURL.toString() %>" data-show-root-folder="<%= Boolean.FALSE.toString() %>" data-show-siblings="<%= Boolean.TRUE.toString() %>" href="<%= viewDocumentsHomeURL.toString() %>">
								<liferay-ui:icon cssClass="expand-folder-arrow" image="../aui/carat-1-r" message="" />
							</a>
						</c:if>

						<a class="browse-folder" data-navigation="documents-home" data-refresh-entries="<%= Boolean.TRUE.toString() %>" data-resource-url="<%= viewDocumentsHomeEntriesURL.toString() %>" data-show-siblings="<%= Boolean.TRUE.toString() %>" href="<%= viewDocumentsHomeURL.toString() %>">
