...
boolean ldapImportUserPasswordAutogenerated = PropsValues.LDAP_IMPORT_USER_PASSWORD_AUTOGENERATED;
boolean ldapExportInputDisabled = ldapImportEnabled && ldapImportUserPasswordAutogenerated;
ldapExportEnabled = ldapExportEnabled && !ldapExportInputDisabled;
...
<aui:fieldset>
	<liferay-ui:error key="ldapAutogenertingImportExport" message="ldap-export-and-import-cant-be-enabled-when-autogenerating-passwords" />
...
<aui:fieldset>
	<c:choose>
		<c:when test="<%= ldapImportUserPasswordAutogenerated %>">
	...
	<aui:input disabled="<%= ldapExportInputDisabled %>" id="ldapExportEnabled" label="export-enabled" name='<%= "settings--" + PropsKeys.LDAP_EXPORT_ENABLED + "--" %>' type="checkbox" value="<%= ldapExportEnabled %>" />
</aui:fieldset>
...
<c:if test="<%= ldapImportUserPasswordAutogenerated %>">
	<aui:script>
	function <portlet:namespace />enableExport() {
		var ldapImportUserPasswordAutogenerated = <%= ldapImportUserPasswordAutogenerated %>;

		var A = AUI();

		var exportInput = A.one('#<portlet:namespace />ldapExportEnabled');
		var importInput = A.one('#<portlet:namespace />ldapImportEnabled');
		...
		var importEnabled = importInput.val() == "true";
		var exportEnabled = exportInput.val() == "true";

		if(importEnabled && ldapImportUserPasswordAutogenerated){
			if(exportEnabled){
				exportCheckbox.simulate('click');
			}
			exportCheckbox.set('disabled', true);
		} else {
			exportCheckbox.set('disabled', false);
		}
