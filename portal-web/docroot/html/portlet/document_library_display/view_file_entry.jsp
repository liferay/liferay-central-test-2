	...
	<liferay-ui:panel-container extended="<%= false %>" persistState="<%= true %>">
		<liferay-ui:panel collapsible="<%= true %>" cssClass="version-history" extended="<%= true %>" persistState="<%= true %>" title="version-history">
			...
			for (int i = 0; i < results.size(); i++) {
				...
				row.addJSP("/html/portlet/document_library_display/file_version_download.jsp");

				// Convert to

				if (conversions.length > 0) {
					row.addJSP("/html/portlet/document_library_display/file_version_convert_to.jsp");
				}
			...
			%>
				...
				<portlet:actionURL var="compareVersionsURL">
					<portlet:param name="struts_action" value="/document_library_display/compare_versions" />
				</portlet:actionURL>
