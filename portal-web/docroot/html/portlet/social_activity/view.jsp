			...
			%>
				...
				<h4 class="social-activity-item" data-modelName="<%= className %>" title="<%= localizedClassName %>">
					<div class="social-activity-item-content">
						<aui:input disabled="<%= !SocialActivityPermissionUtil.contains(permissionChecker, themeDisplay.getSiteGroupId(), ActionKeys.CONFIGURATION) %>" hideLabel="<%= true %>" inlineField="<%= true %>" label="enabled" name='<%= className + ".enabled" %>' title="" type="checkbox" value="<%= enabled %>" />
