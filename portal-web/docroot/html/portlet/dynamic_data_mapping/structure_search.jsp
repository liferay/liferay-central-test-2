	...
	<aui:fieldset cssClass="lfr-ddm-search-form">
		...
		<aui:column>
			<aui:select label="type" name="<%= displayTerms.CLASS_NAME_ID %>">
				<aui:option label='<%= "model.resource." + DDLEntry.class.getName() %>' selected='<%= "datalist".equals(displayTerms.getStorageType()) %>' value="<%= PortalUtil.getClassNameId(DDLEntry.class.getName()) %>" />
			</aui:select>
		</aui:column>
