...
FormStructure formStructure = (FormStructure)request.getAttribute(WebKeys.FORM_STRUCTURE);

long groupId = BeanParamUtil.getLong(formStructure, request, "groupId", scopeGroupId);

String formStructureId = BeanParamUtil.getString(formStructure, request, "formStructureId");
String newFormStructureId = ParamUtil.getString(request, "newFormStructureId");

String xsd = BeanParamUtil.getString(formStructure, request, "xsd");
%>

<liferay-portlet:actionURL var="editFormURL" portletName="<%= PortletKeys.FORMS %>">
	<portlet:param name="struts_action" value="/forms/edit_form_structure" />
...
<aui:form action="<%= editFormURL %>" method="post" name="fm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "saveForm();" %>'>
	...
	<aui:input name="formStructureId" type="hidden" value="<%= formStructureId %>" />
	<aui:input name="xsd" type="hidden" />

	<liferay-ui:error exception="<%= FormDuplicateStructureElementException.class %>" message="please-enter-unique-structure-field-names-(including-field-names-inherited-from-the-parent-structure)" />
	<liferay-ui:error exception="<%= FormDuplicateStructureIdException.class %>" message="please-enter-a-unique-id" />
	<liferay-ui:error exception="<%= FormStructureIdException.class %>" message="please-enter-a-valid-id" />
	<liferay-ui:error exception="<%= FormStructureNameException.class %>" message="please-enter-a-valid-name" />
	<liferay-ui:error exception="<%= FormStructureXsdException.class %>" message="please-enter-a-valid-xsd" />

	<aui:model-context bean="<%= formStructure %>" model="<%= FormStructure.class %>" />

	<aui:input name="name" />

	<liferay-ui:panel-container cssClass="lfr-form-structure-details-container" extended="<%= false %>" id="formStructureDetailsPanelContainer" persistState="<%= true %>">
		<liferay-ui:panel collapsible="<%= true %>" extended="<%= false %>" id="formStructureDetailsSectionPanel" persistState="<%= true %>" title='<%= LanguageUtil.get(pageContext, "details") %>'>
			<c:choose>
				<c:when test="<%= formStructure == null %>">
					<c:choose>
						<c:when test="<%= PropsValues.FORM_STRUCTURE_FORCE_AUTOGENERATE_ID %>">
							<aui:input name="newFormStructureId" type="hidden" />
							<aui:input name="autoFormStructureId" type="hidden" value="<%= true %>" />
						</c:when>
						<c:otherwise>
							<aui:input cssClass="lfr-input-text-container" field="formStructureId" fieldParam="newFormStructureId" label="id" name="newFormStructureId" value="<%= newFormStructureId %>" />

							<aui:input label="autogenerate-id" name="autoFormStructureId" type="checkbox" value="<%= true %>" />
						</c:otherwise>
					</c:choose>
				</c:when>
				<c:otherwise>
					...
					<c:otherwise>
						<li class="aui-component aui-form-builder-field aui-widget" id="<portlet:namespace />defaultField">
							<div class="aui-form-builder-field-content">
								<label class="aui-field-label"><liferay-ui:message key="text-input" /></label>
					...
					<div class="aui-tabview-content-item">
						<ul class="aui-form-builder-drag-container">
							<li class="aui-form-builder-drag-node aui-form-builder-field" data-type="text">
								...
								<span class="aui-form-builder-label"><liferay-ui:message key="text" /></span>
							</li>
...
<aui:script use="liferay-portlet-forms">
					...
					[
						{
							...
							label: '<liferay-ui:message key="text-input" />',
	...
	Liferay.provide(
		...
		function() {
			...
			<c:if test="<%= formStructure == null %>">
				document.<portlet:namespace />fm.<portlet:namespace />formStructureId.value = document.<portlet:namespace />fm.<portlet:namespace />newFormStructureId.value;
			</c:if>
	...
	<c:if test="<%= Validator.isNotNull(formStructureId) %>">
		window.parent.<%= HtmlUtil.escapeJS(callback) %>('<%= HtmlUtil.escapeJS(formStructureId) %>');
	</c:if>
</aui:script>