...
<span class="<%= _buildCss(BUTTON_PREFIX, type, false, disabled, false, false, false, cssClass, null) %>">
	<span class="aui-button-content">
		<input class="<%= _buildCss(BUTTON_INPUT_PREFIX, type, false, false, false, false, false, null, null) %>" <%= disabled ? "disabled" : StringPool.BLANK %> <%= Validator.isNotNull(name) ? "id=\"" + namespace + name + "\"" : StringPool.BLANK %> <%= Validator.isNotNull(onClick) ? "onClick=\"" + onClick + "\"" : StringPool.BLANK %> type='<%= type.equals("cancel") ? "button" : type %>' value="<%= LanguageUtil.get(pageContext, value) %>" <%= _buildDynamicAttributes(dynamicAttributes) %> />
	</span>
</span>