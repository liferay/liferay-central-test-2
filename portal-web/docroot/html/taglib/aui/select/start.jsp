...
<div class="<%= controlGroupCss %>">
	...
	<select class="<%= fieldCss %>" <%= disabled ? "disabled" : StringPool.BLANK %> id="<%= namespace + id %>" <%= multiple ? "multiple" : StringPool.BLANK %> name="<%= namespace + name %>" <%= Validator.isNotNull(onChange) ? "onChange=\"" + onChange + "\"" : StringPool.BLANK %><aui:spacer /><%= Validator.isNotNull(onClick) ? "onClick=\"" + onClick + "\"" : StringPool.BLANK %><aui:spacer /><%= Validator.isNotNull(title) ? "title=\"" + title + "\"" : StringPool.BLANK %><aui:spacer /><%= AUIUtil.buildData(data) %><aui:spacer /><%= InlineUtil.buildDynamicAttributes(dynamicAttributes) %>>
