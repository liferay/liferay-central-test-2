...
<c:if test="<%= type != null %>">
	<c:choose>
		...
		<c:when test='<%= type.equals("double") || type.equals("int") || type.equals("long") || type.equals("String") %>'>
			...
			String fieldXml = StringPool.BLANK;

			if (localized){
				if(Validator.isNotNull(bean)) {
					fieldXml = BeanPropertiesUtil.getString(bean, field);
				}
				else {
