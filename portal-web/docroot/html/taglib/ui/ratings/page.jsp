							...
							<c:choose>
								<c:when test="<%= TrashUtil.isInTrash(className, classPK) || !themeDisplay.isSignedIn() %>">
									...
									<c:if test='<%= type.equals("thumbs") %>'>
										<span class="glyphicon glyphicon-thumbs-down icon-thumbs-down rating-element rating-thumb-down rating-<%= (yourScore == 0) ? "on" : "off" %>" title="<liferay-ui:message key="ratings-are-disabled-because-this-entry-is-in-the-recycle-bin" />"><%= negativeVotes %></span>
									</c:if>
								</c:when>
								<c:otherwise>
									<a class="glyphicon glyphicon-thumbs-up icon-thumbs-up rating-element rating-thumb-up rating-<%= (yourScore > 0) ? "on" : "off" %>" href="javascript:;"><%= positiveVotes %></a>

									<c:if test='<%= type.equals("thumbs") %>'>
										<a class="glyphicon glyphicon-thumbs-down icon-thumbs-down rating-element rating-thumb-down rating-<%= (yourScore == 0) ? "on" : "off" %>" href="javascript:;"><%= negativeVotes %></a>
									</c:if>
