 ...
 */
public class AddVocabulary3Test extends BaseTestCase {
	public void testAddVocabulary3() throws Exception {
		...
		selenium.type("//input[@id='_147_title_en_US']",
			RuntimeVariables.replace("Vocabulary3 Name"));
		selenium.type("//textarea[@id='_147_description_en_US']",
			RuntimeVariables.replace("Vocabulary3 Description"));
		...
		for (int second = 0;; second++) {
			...
			try {
				if (RuntimeVariables.replace("Vocabulary3 Name")
										.equals(selenium.getText(
								"xPath=(//span[@class='vocabulary-item']/a)[3]"))) {
					break;
				}
			}
			catch (Exception e) {
