...
public class AssertEmailNotificationPasswordChangedTest extends BaseTestCase {
	public void testAssertEmailNotificationPasswordChanged()
		...
		for (int second = 0;; second++) {
			...
			try {
				if (selenium.isVisible("link=Password Changed Notification")) {
					break;
				}
			}
			catch (Exception e) {
		...
		for (int second = 0;; second++) {
			...
			try {
				if (selenium.isVisible(
							"//div[5]/div[3]/fieldset/div/span/span/label")) {
					break;
				}
			}
			catch (Exception e) {
		...
		assertEquals(RuntimeVariables.replace("Subject"),
			selenium.getText("//div[5]/div[3]/fieldset/div/span/span/label"));
		assertEquals(RuntimeVariables.replace("Body"),
			selenium.getText("//div[3]/fieldset/div/div[1]/div/label"));
	}
}