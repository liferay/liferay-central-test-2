...
public class AddFormFieldRadioTest extends BaseTestCase {
	public void testAddFormFieldRadio() throws Exception {
		...
		for (int second = 0;; second++) {
			...
			try {
				if (selenium.isVisible("_86_fieldType7")) {
					break;
				}
			}
			catch (Exception e) {
		...
		selenium.select("_86_fieldType7",
		...
		for (int second = 0;; second++) {
			...
			try {
				if (selenium.isElementPresent("_86_fieldOptions7")) {
					break;
				}
			}
			catch (Exception e) {
		...
		selenium.type("_86_fieldLabel7", RuntimeVariables.replace("Gender"));
		selenium.type("_86_fieldOptions7",
