...
public class SA_AssignUserRolesTest extends BaseTestCase {
	public void testSA_AssignUserRoles() throws Exception {
		...
		for (int second = 0;; second++) {
			...
			try {
				if (RuntimeVariables.replace("BA")
		...
		selenium.clickAt("//tbody/tr[3]/td[2]/a", RuntimeVariables.replace("BA"));
		...
		for (int second = 0;; second++) {
			...
			try {
				if (selenium.isVisible("//a[@id='_125_rolesLink']")) {
					break;
				}
			}
			catch (Exception e) {
		...
		for (int second = 0;; second++) {
			...
			try {
				if (selenium.isVisible("link=Blogs Administrator")) {
					break;
				}
			}
			catch (Exception e) {
		...
		for (int second = 0;; second++) {
			...
			try {
				if (RuntimeVariables.replace("Member")
										.equals(selenium.getText(
								"//tbody/tr[3]/td[2]/a"))) {
					break;
				}
