 ...
 */
public class DeleteLivePageStagingLocalLiveDLDocumentDocxDockTest
	extends BaseTestCase {
	public void testDeleteLivePageStagingLocalLiveDLDocumentDocxDock()
		throws Exception {
		...
		assertFalse(selenium.isElementPresent(
				"//body[@class='blue live-view controls-visible signed-in public-page dockbar-ready']"));
		assertTrue(selenium.isElementPresent("link=View Live Page"));
		assertEquals(RuntimeVariables.replace("DL Document Title.docx"),
			selenium.getText("//td[1]/a"));
		assertEquals(RuntimeVariables.replace("9.6k"),
			selenium.getText("//td[2]/a"));
		...
		for (int second = 0;; second++) {
			...
			try {
				if (selenium.isVisible("//span[1]/span/span/input")) {
					break;
				}
			}
			catch (Exception e) {
