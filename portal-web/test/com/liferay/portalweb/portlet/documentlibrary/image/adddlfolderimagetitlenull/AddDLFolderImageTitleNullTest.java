...
package com.liferay.portalweb.portlet.documentlibrary.image.adddlfolderimagetitlenull;
 ...
 */
public class AddDLFolderImageTitleNullTest extends BaseTestCase {
	public void testAddDLFolderImageTitleNull() throws Exception {
		...
		assertEquals(RuntimeVariables.replace("test_image.jpg"),
			selenium.getText(
				"//a[contains(@class,'document-link')]/span[@class='entry-title']"));
		selenium.clickAt("//a[contains(@class,'document-link')]/span[@class='entry-title']",
			RuntimeVariables.replace("test_image.jpg"));
		selenium.waitForPageToLoad("30000");
		...
		for (int second = 0;; second++) {
			...
			try {
				if (RuntimeVariables.replace("test_image.jpg")
										.equals(selenium.getText(
