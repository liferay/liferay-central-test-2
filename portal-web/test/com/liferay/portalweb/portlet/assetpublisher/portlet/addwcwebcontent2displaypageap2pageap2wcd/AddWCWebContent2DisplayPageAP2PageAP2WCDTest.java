 ...
 */
public class AddWCWebContent2DisplayPageAP2PageAP2WCDTest extends BaseTestCase {
	public void testAddWCWebContent2DisplayPageAP2PageAP2WCD()
		throws Exception {
		...
		for (int second = 0;; second++) {
			...
			try {
				if (selenium.isVisible(
							"//a[@id='treeContainerPublicPages_layout_asset-publisher-test-page2']")) {
					break;
				}
			}
			catch (Exception e) {
		...
		assertEquals(RuntimeVariables.replace("Asset Publisher Test Page2"),
			selenium.getText(
				"//a[@id='treeContainerPublicPages_layout_asset-publisher-test-page2']"));
		selenium.clickAt("//a[@id='treeContainerPublicPages_layout_asset-publisher-test-page2']",
			RuntimeVariables.replace("Asset Publisher Test Page2"));
