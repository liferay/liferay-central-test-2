 ...
 */
public class ReplyMBThreadMessageReply2Test extends BaseTestCase {
	public void testReplyMBThreadMessageReply2() throws Exception {
		...
		selenium.type("//td[@id='cke_contents__19_editor']/textarea",
			RuntimeVariables.replace("MB Thread Message Reply2 Body"));
		assertEquals(RuntimeVariables.replace("Source"),
			selenium.getText("//span[.='Source']"));
		selenium.clickAt("//span[.='Source']",
			RuntimeVariables.replace("Source"));
		...
		for (int second = 0;; second++) {
			...
			try {
				if (RuntimeVariables.replace("MB Thread Message Reply2 Body")
