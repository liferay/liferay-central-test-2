 ...
 */
public class ViewWCTemplateStructureFieldDecimalTest extends BaseTestCase {
	public void testViewWCTemplateStructureFieldDecimal()
		throws Exception {
		...
		assertEquals(RuntimeVariables.replace(
			...
			selenium.getText(
				"//tr[contains(.,'WC Template Structure Decimal Name')]/td[3]/a"));
		selenium.clickAt("//tr[contains(.,'WC Template Structure Decimal Name')]/td[3]/a",
			RuntimeVariables.replace("WC Template Structure Decimal Name"));
		selenium.waitForPageToLoad("30000");
		Thread.sleep(1000);
		selenium.waitForVisible("//h1[@class='header-title']/span");
		assertEquals(RuntimeVariables.replace(
				"WC Template Structure Decimal Name (WC Structure Decimal Name)"),
			selenium.getText("//h1[@class='header-title']/span"));
		assertEquals("WC Template Structure Decimal Name",
			selenium.getValue("//input[@id='_166_name_en_US']"));
		assertEquals("WC Template Structure Decimal Description",
		...
		assertEquals(RuntimeVariables.replace("<p>$decimal.getData()</p>##"),
			selenium.getText(
				"//div[@class='ace_layer ace_text-layer']/div/div[contains(.,'decimal.getData')]"));
		selenium.selectFrame("relative=top");
	}
}