 ...
 */
public class ViewWCTemplateStructureFieldDMTest extends BaseTestCase {
	public void testViewWCTemplateStructureFieldDM() throws Exception {
		...
		assertEquals(RuntimeVariables.replace("WC Template Structure DM Name"),
			selenium.getText(
				"//tr[contains(.,'WC Template Structure DM Name')]/td[3]/a"));
		selenium.clickAt("//tr[contains(.,'WC Template Structure DM Name')]/td[3]/a",
			RuntimeVariables.replace("WC Template Structure DM Name"));
		selenium.waitForPageToLoad("30000");
		Thread.sleep(1000);
		selenium.waitForVisible("//h1[@class='header-title']/span");
		assertEquals(RuntimeVariables.replace(
				"WC Template Structure DM Name (WC Structure DM Name)"),
			selenium.getText("//h1[@class='header-title']/span"));
		assertEquals("WC Template Structure DM Name",
			selenium.getValue("//input[@id='_166_name_en_US']"));
		assertEquals("WC Template Structure DM Description",
		...
		assertEquals(RuntimeVariables.replace("<p>$dm.getData()</p>##"),
			selenium.getText(
				"//div[@class='ace_layer ace_text-layer']/div/div[contains(.,'dm.getData')]"));
		selenium.selectFrame("relative=top");
	}
}