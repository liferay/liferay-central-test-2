 ...
 */
public class AddPermissionAccessInControlPanelTest extends BaseTestCase {
	public void testAddPermissionAccessInControlPanel()
		throws Exception {
		int label = 1;
		...
		while (label >= 1) {
			...
			case 1:
				...
				boolean webContentAccessControlPanel = selenium.isChecked(
						"//input[@value='15ACCESS_IN_CONTROL_PANEL']");

				if (webContentAccessControlPanel) {
					label = 2;
				...
				selenium.clickAt("//input[@value='15ACCESS_IN_CONTROL_PANEL']",
					RuntimeVariables.replace("Access In Control Panel Checkbox"));
